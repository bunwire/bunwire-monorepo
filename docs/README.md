...
Core exposes `runInvocation()` as the generic managed-invocation boundary. It creates an isolated child container and real `InvocationContext`, applies invocation-local bindings, runs Provider `boot(context)`, and only then calls the invocation handler. Adapters may supply `ManagedInvocationOptions.parentContainer`, provided it is rooted in the same Application, so an adapter-owned request/job scope becomes the parent of Core's invocation child without leaking contextual bindings into the root. This is lifecycle/scope orchestration rather than automatic exposure of Service or Controller methods.
...
The analyzer resolves registered decorator and injector exports to their canonical TypeScript symbols. Aliases and re-exports remain valid because they resolve to the same symbol; same-ID definitions from any other symbol fail closed. Explicit `@Inject()` expressions must resolve to a constructable class or `createToken()` value rather than merely to an arbitrary runtime value.

Managed subclasses with implicit constructors are accepted only when their effective inherited constructor has no parameters. A subclass inheriting parameters must declare an explicit forwarding constructor so every dependency source and index is visible to analysis. Statically known managed-class constructor cycles are rejected with the dependency path. Managed methods must have concrete instance implementations; static, abstract, and declaration-only decorated methods are invalid.
