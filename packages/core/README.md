...
Adapters may contribute `eventListenerDelivery(context, next)` to delegate selected listeners to platform facilities. Its frozen context contains the exact listener/event and existing invocation. `next()` is single-use and invokes the direct listener in that same scope. Core awaits any started direct continuation, preserves distinct interceptor/handler errors, and skips later listeners on failure. Explicit dispatcher replacement bypasses the hook. Core does not implement queue policies, serialization, priorities, concurrent fan-out or alias/string dispatch.

`defineManagedClassAttachmentDecorator()` defines supplementary class metadata without introducing another class kind. Adapters contribute definitions through `classAttachments`; each declares `allowedOn`, a canonical compiler symbol and a pure `createMetadata` callback. Attachments support either decorator order but cannot be duplicated, misplaced, forged or inherited. Generated `defineManagedClassAttachment()` records live in the optional/default-empty runtime registry sidecar, preserving the exact original class/event/listener records. Core validates matching own metadata and complete generated delivery before Providers run. Plain legacy-decorated classes are rejected by compilation/generated-record validation because the outer managed decorator may not have executed when the attachment is applied.
...
`InvocationEngine` consumes the prebuilt plan without inspecting source or reclassifying parameters. It requires both the owning class kind and method kind to use their registered canonical descriptors, validates caller counts, resolves the declared sources, invokes the managed method directly, and returns a Promise-normalized result. `Application.invokeManagedMethod()` executes that engine through `runInvocation()`, preserving invocation scope and Provider boot ordering. Unknown method kinds, resolver IDs, and malformed plans fail with dedicated actionable errors.
...
`ManagedMethodPlan.middleware` contains only canonical attachment records. Runtime plan validation checks every attachment through the managed-middleware boundary. Adapters select applicable attachments and execute them through `ManagedInvocationOptions.around`; the generic invocation engine does not interpret adapter filters or transports.
...
`ManagedInvocationOptions.around` wraps the terminal managed invocation after Provider `boot()`. This allows an adapter to run `executeMiddlewareChain()` around `invokeManagedMethod()` without creating a second invocation scope. Function middleware and callback overloads are not supported.
...
`Application.withSchedule(callback)` is the equivalent compile-only boundary for platform-neutral scheduling declarations. Its typed registry supports existing managed job/task targets and cadence builders, but Core never executes the callback or interprets cron. Vite and an adapter-supplied generic schedule compilation handler emit frozen `RuntimeScheduleDefinition` records; the owning runtime package supplies execution, timezone, and lock semantics. Runtime registry validation ensures schedule IDs are unique and every target is a canonical generated class.
...
`defineRuntimeRegistry()` is the compiler/runtime contract for generated and prebuilt metadata. It carries managed class entries with scope and indexed constructor dependencies, a generated Provider list, complete managed-method plans, canonical events/listeners, the event-alias index, and optional platform-neutral schedule records. Application startup validates canonical descriptor/decorator identity, shared event/listener record identity, schedule identity, and schedule targets; registers constructor metadata and convention class bindings; installs the default dispatcher; then runs generated and explicitly contributed Providers through the same lifecycle. Registry consumers connect metadata only after Provider registration. Runtime never scans source or reclassifies parameters.

Adapters may contribute `ManagedClassCompilationHandlerData` through `defineCompilerMetadataHandler()`. The `bunwire.managed-class-compilation` contract supplies protected literal property names, a pure metadata compiler callback, singleton/transient scope, and intrinsic public method names with canonical method-kind exports. Each intrinsic method accepts no parameters, payload-only transport parameters, or an exact allowlist of adapter parameter-resolver IDs. Vite emits ordinary class records and invocation plans. Core permits undecorated intrinsic methods only for those exact declared class/kind/name combinations, with canonical class decorators, required handler presence, the declared binding scope, own concrete methods, and the declared parameter policy. Other methods still require canonical managed-method decorators. No compiler callback runs during Application startup, and no new binding lifetime is introduced.
